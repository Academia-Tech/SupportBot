...
We evaluate SupportBot on \supportbench{} using an offline replay protocol that replicates real-time production behavior. On the Ardupilot-UA dataset, SupportBot achieves a Score of 8.5/10 (quality $\times$ recall) with 93\% precision and 100\% recall, outperforming a standard chunked-RAG baseline (6.3, +22\%). \supportbench{} provides the first public benchmark specifically designed for evaluating case-mining support bots across languages and domains.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Empirical validation}: Evaluation on \supportbench{} showing SupportBot achieves a Score of 8.5/10 with 93\% precision and 100\% recall, outperforming a chunked-RAG baseline by 35\%.
\end{enumerate}
...
\midrule
SupportBot      & \textbf{8.5} & \textbf{8.5} & \textbf{93.2} & \textbf{100.0} \\
Chunked-RAG     & 6.3 & 7.2 & 85.7 & 88.2 \\
\bottomrule
\end{tabular}
\caption{\supportbench{} results on Ardupilot-UA. \textbf{Score}: quality $\times$ recall (0--10). \textbf{Quality}: mean per-response rating (0--10). \textbf{Prec.}/\textbf{Rec.}: precision and recall (\%) against LLM-identified ground-truth questions. Both systems use Gemini 2.5 Pro for generation.}
\label{tab:supportbench-results}
\end{table}

Table~\ref{tab:supportbench-results} presents results on the Ardupilot-UA dataset (Ukrainian UAV/drone support community). SupportBot achieves a Score of 8.5/10, outperforming chunked-RAG (6.3, +22\%).

\paragraph{Quality advantage.} SupportBot achieves 8.5/10 quality (correctness=9.5, helpfulness=7.9, specificity=8.4, necessity=8.0) compared to 7.2 for chunked-RAG. The gap is driven by SupportBot's ability to retrieve verified community solutions and augment them with web search, rather than relying on generic chunk retrieval that loses conversational context.

\paragraph{Precision.} SupportBot's gate classifier filters non-questions before synthesis, achieving 93.2\% precision. Chunked-RAG (85.7\%) over-responds to statements, helper advice, and acknowledgments.

\paragraph{Recall through escalation.} SupportBot achieves 100\% recall by escalating questions it cannot confidently answer to human admins. Chunked-RAG achieves 88.2\% recall (4 missed), without escalation capability.
...
On \supportbench{}, SupportBot achieves a Score of 8.5/10 with 93\% precision and 100\% recall on Ardupilot-UA, outperforming a standard chunked-RAG system (6.3, +22\%). To enable broader evaluation, we release \supportbench{}, a multilingual benchmark of 60{,}000 messages from six technical support communities across three languages and six domains.
